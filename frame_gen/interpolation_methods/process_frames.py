...
def film_interpolation(frame1, frame3, model):
    ...
    print(f"Model output type: {type(output_dict)}")
...
if __name__ == "__main__":
    # Make sure you have tensorflow and tensorflow-hub installed:
    # pip install tensorflow tensorflow-hub opencv-python numpy
    
    # For better memory management with large frames
    gpus = tf.config.experimental.list_physical_devices('GPU')
    if gpus:
        try:
            # Limit TensorFlow to allocate only what's needed
            for gpu in gpus:
